...
import Utilities.WebElementObjects;
				...
				if (i % 2 != 0 && j > 1) {
					...
					if (eliminateObjects.equals(eliminateObjects.eliminateLemonGrass)) {
						...
						if (strValue.contains("tsp") || strValue.contains("tbsp")) {
							WebElement prepmethodanotherdiv = driver
									.findElement(By.xpath("//div[12]/section/div/div[1]"));
						...
						driver.navigate().back();
						Thread.sleep(1000);
						WebElement receipeid = driver.findElement(By.xpath(
								"//div[3]/div[1]/div[2]/span[normalize-space(contains (text(),'Recipe#')) and not(@br)]"));
					...
					/*
					 * String regex = "\\d+"; Pattern pattern = Pattern.compile(regex); Matcher
					 * matcher = pattern.matcher(strUrl); if (matcher.find()) { int Receipeid =
					 * Integer.parseInt(matcher.group()); String Receipeidstr =
					 * Integer.toString(Receipeid); writeOutput.setCellData("ALLERGENICS",
					 * setCellvalue, 0, Receipeidstr);
					 */
				}
			}
